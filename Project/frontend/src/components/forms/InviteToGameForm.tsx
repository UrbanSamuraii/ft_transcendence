...
export const InviteToGameForm: React.FC<MemberInConversationFormProps> = ({ setShowModal }) => {
    ...
    const sendInvitationGame = async (username: string) => {
        try {
            const response = await axios.post(`http://${server_adress}:3001/users/invite_game`,
                    ...
                    {memberList.map((member) => (
                        <li key={member.username}>
                            <button
                                ...
                                onClick={() => sendInvitationGame(member.username)}
                            >
                                {member.username}
                            </button>
                        </li>
